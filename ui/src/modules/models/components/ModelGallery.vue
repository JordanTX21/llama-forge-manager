...
import { Cpu, Database, Settings, Image, Play } from '@lucide/vue'
  ...
  <div v-else-if="!isListView" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
    ...
    <div v-for="model in models" :key="model.path"
      ...
      <div class="h-28 bg-surface-container-high relative overflow-hidden flex items-center justify-center">
        <component :is="isVisionEncoder(model) ? Image : Cpu" class="w-12 h-12 text-outline opacity-30 group-hover:opacity-50 transition-opacity group-hover:scale-110 duration-500" />
        <div class="absolute top-3 right-3 z-10">
          <span v-if="runningModels.has(model.filename)" class="bg-blue-500/10 text-blue-400 text-[10px] font-bold px-2 py-0.5 rounded border border-blue-500/20 flex items-center gap-1 animate-pulse">
            <span class="w-1 h-1 rounded-full bg-blue-400"></span> {{ t('models.running') }}
          </span>
          <span v-else class="bg-green-500/10 text-green-500 text-[10px] font-bold px-2 py-0.5 rounded border border-green-500/20 flex items-center gap-1">
      ...
      <div class="p-4 flex-1 flex flex-col">
        <div class="flex justify-between items-start mb-3 gap-3">
          <div class="flex-1 min-w-0">
            ...
            <span class="text-xs text-on-surface-variant truncate block" :title="model.author">{{ t('models.by', { author: model.author || t('models.local') }) }}</span>
          </div>
          <span v-if="isVisionEncoder(model)" class="font-label text-[10px] text-purple-400 bg-purple-500/10 border border-purple-500/20 px-1.5 py-0.5 rounded shrink-0">MMPROJ</span>
          <span v-else class="font-label text-[10px] text-primary bg-primary-container/20 px-1.5 py-0.5 rounded shrink-0">{{ model.filename.split('.').pop()?.toUpperCase() || 'GGUF' }}</span>
        </div>
        <div class="flex items-center justify-between mt-auto pt-3 border-t border-outline/50">
          ...
          <div class="flex gap-2">
            ...
            <button v-if="!isVisionEncoder(model)" @click.stop="goToSettings(model)" class="p-1.5 rounded-full hover:bg-surface-container-high text-on-surface-variant hover:text-primary transition-colors">
  ...
  <div v-else class="flex flex-col gap-3">
    <div v-for="model in models" :key="model.path"
      ...
      <div class="w-12 h-12 rounded-lg bg-surface-container-high flex items-center justify-center shrink-0">
        <component :is="isVisionEncoder(model) ? Image : Cpu" class="w-6 h-6 text-outline opacity-60 group-hover:opacity-100 transition-opacity" />
      </div>
      ...
      <div class="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
        ...
        <div class="hidden md:flex col-span-2 items-center">
          <span v-if="isVisionEncoder(model)" class="font-label text-[10px] text-purple-400 bg-purple-500/10 border border-purple-500/20 px-2 py-1 rounded shrink-0">MMPROJ</span>
          <span v-else class="font-label text-[10px] text-primary bg-primary-container/20 px-2 py-1 rounded shrink-0">{{ model.filename.split('.').pop()?.toUpperCase() || 'GGUF' }}</span>
        </div>

        <div class="hidden md:flex col-span-2 items-center justify-end gap-2">
          <span v-if="runningModels.has(model.filename)" class="bg-blue-500/10 text-blue-400 text-[10px] font-bold px-2 py-1 rounded border border-blue-500/20 flex items-center gap-1 animate-pulse">
            <span class="w-1 h-1 rounded-full bg-blue-400"></span> {{ t('models.running') }}
          </span>
          <span v-else class="bg-green-500/10 text-green-500 text-[10px] font-bold px-2 py-1 rounded border border-green-500/20 flex items-center gap-1">
            <span class="w-1 h-1 rounded-full bg-green-500"></span> {{ t('models.ready') }}
          </span>
          <button v-if="!isVisionEncoder(model) && getCommandForModel(model)" @click.stop="runModel(model)"
            class="p-1.5 rounded-full hover:bg-green-500/10 text-on-surface-variant hover:text-green-400 transition-colors"
